...
namespace CMTVEngine
{
    ...
    public static class BitTools
    {
        ...
        public static void CopyToBytesArray(byte[] source, byte[] des, int len)
        {
            CopyToBytesArray(source, 0, des, len);          
        }

        public static void CopyToBytesArray(byte[] source, int offset, byte[] des, int len)
        {
            for (int i = 0; i < len; i++)
                des[i] = source[i + offset];
        }

        public static void CopyFromStreamToBytesArray(Stream source, long startAddr, int len, ref byte[] des)
        {
            //参数是对象的话默认是引用传递
            long pos_backup = source.Position;
            source.Seek(startAddr, SeekOrigin.Begin);
            if (des == null)
                des = new byte[len];
            source.Read(des, 0, len);
            //restore backup
            source.Seek(pos_backup, SeekOrigin.Begin);
        }

        public static Stream CopyToNewStream(Stream source, long startAddr, int len)
        {
            //参数是对象的话默认是引用传递
            long pos_backup = source.Position;
            ...
            m.Write(reader.ReadBytes(len), 0, len);
